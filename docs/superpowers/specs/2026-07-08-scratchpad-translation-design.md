...
用户**只跑一条命令**（`postman-chinese-injector`，或双击二进制），本脚本一次性把
**两个钩子都注入**进同一个 app；用户无需关心自己是哪个版本、登没登入。两钩子在**运行时
各自守卫**、自动生效于适用场景：

| 运行环境 | 生效钩子 | 生效方式 |
|---------|---------|---------|
| 新版 Postman（远程 web） | `pm-chinese.js`（fetch 拦截） | 拦 `_ar-assets/locales` 换中文 |
| 老版登入态（远程 web） | `pm-chinese.js`（fetch 拦截） | 同上 |
| 老版登出态 Scratch Pad（本地 bundle） | `pm-scratchpad-cn.js`（DOM 翻译） | 精确匹配换可见文本 |

两钩子互不干扰：fetch 钩子看有没有 locale 请求；DOM 钩子看是不是 `file://…scratchpad` 窗口。
在不适用的环境下对应钩子静默不动（如新版无本地 Scratch Pad 时 DOM 钩子守卫不通过）。
