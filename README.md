...
**① 改 `Preferences.json`（首选，不影响下载）**（先完全退出 Postman）
...
Postman 经 `GET dl.pstmn.io/update/status?...` 检查更新，并从**同一个** `dl.pstmn.io` 下载更新包。hosts 是**按域名**生效、无法只挡更新而放行下载，所以屏蔽这行后**上面那些手动下载链接也会一起失效**——这正是它「彻底」的代价，也是为什么把它排在 ① 之后当兜底。
