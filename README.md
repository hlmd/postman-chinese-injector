...
- **桌面端**：重打包 `app.asar`——Electron 资源解析时 `app.asar` 存在则优先、忽略 `resources/app/`
  文件夹。CLI 会自动备份原始 asar、从备份打补丁、再打包回去（幂等，可还原）。
...
> 探测不到时用 `--resources <含 app.asar 的目录>` 或 `--postman-dir <安装根目录>` 指定。
